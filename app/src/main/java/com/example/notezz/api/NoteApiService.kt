...
interface NoteApiService {
    // All note
    @GET("notes")
    suspend fun fetchAllNote(@Header("Authorization") token: String): Response<NoteResponse>
    // Find note by id
    @POST("notes/{id}")
    suspend fun fetchNoteById(@Header("Authorization") token: String,
                              @Path("id") id: String): Response<NoteResponse>
    // Create new note
    @POST("notes")
    ...
    suspend fun updateNote(@Header("Authorization") token: String,
                           @Body updateNoteModel: UpdateNoteModel): Response<NoteResponse>
    // Delete note
    @DELETE("notes/{id}")
    suspend fun deleteNote(@Body accessToken: String): Response<NoteResponse>
}